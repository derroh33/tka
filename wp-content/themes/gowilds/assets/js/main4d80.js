		...
		scrollTop: function(){
	 	...
	 	booking: function(){
			...
			$('#checkout_form_block > *:eq(0), #checkout_form_block > *:eq(1)').wrapAll('<div class="checkout-items"></div>');
	 		...
	 		$('.booking-single-gallery').each(function(){
	            var image_count = $(this).find('.images-count').html();
	            $(this).find('.wte-trip-image-gal-popup-trigger').html('<span>'+image_count+'</span>');
	        });

	        if($('#modal-trip-send-enquiry-mess').length){
	            var myModal = new bootstrap.Modal(document.getElementById('modal-trip-send-enquiry-mess'));
	            $('#wte-send-enquiry-message, .open-enquiry-message').on('click', function(e){
	               e.preventDefault();
	               myModal.show();
	            });
	            $('.modal-trip-close').on('click', function(){
	               myModal.hide();
	            })
	        }
	 	},
         ...
         $('.listing-total-reviews').find('.review__progress-bar').each(function(){
